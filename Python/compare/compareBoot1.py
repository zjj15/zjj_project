...
#打开上周/本周release相关文件
f1 =  open('lastWeekFiles\installed-files-boot.txt', 'r+',encoding='utf-8')
f2 =  open('thisWeekFiles\installed-files-boot.txt', 'r+',encoding='utf-8')

mywb = openpyxl.Workbook()
mywb.create_sheet(title='result_boot',index = 0)
mysheet = mywb.get_sheet_by_name('result_boot')
...
for i in range(0,len(tmp1File)):
    ...
    mysheet.cell(i+2,3).value = (tmp1File[i][0:14])
...
for i in range(0,len(thisNeedRemoveIndex)):
    ...
    mysheet.cell(lenEqual+2+i,3).value = (tmp1File[i][0:14])
