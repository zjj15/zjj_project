...
if (not exists(Template('../../../Design/Home_mark_focus.png'), timeout=2,threshold=0.90, device=DEV1) and not exists(Template('../../../Design/Home_mark_nofocus.png'), timeout=2,threshold=0.90, device=DEV1)):
    wait_for_appearance(Template('../../../Design/Home_Page1_Bar.png'), timeout=60, device=DEV1, raise_error=False)

do_segment(Segment('../../../Common/BackHomeView.tcs'))
try:
    assert_exists(Template('../../../Design/statusbar_BTA.png'), threshold=0.90, device=DEV1)
    rect_target_area=[538, 112, 390, 540]#rect_target_area=poco('android.widget.ImageView', nearest=({'text':'音乐'})).get_attr('rect')
