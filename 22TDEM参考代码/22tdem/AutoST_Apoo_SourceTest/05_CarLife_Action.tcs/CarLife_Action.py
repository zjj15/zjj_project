...
checkImage_night = 'pic/CarLife_Menu_night.png'
checkImage_day = 'pic/CarLife_Menu.png' #ill off
...
def checkDisp(imageType='day'):
    if imageType == 'night':
        if exists(Template(checkImage_night), timeout=1, device=DEV1):
            return True
        else:
            return False
    else:
        if exists(Template(checkImage_day), timeout=1, device=DEV1):
            return True
        else:
            return False
...
touch(Template("pic/CarLifeBtn.png"), device=DEV1)
if exists(Template('pic/CarLife_Menu.png'), timeout=10, device=DEV1):
    test()
else:
    error('CarLife disconnect')
