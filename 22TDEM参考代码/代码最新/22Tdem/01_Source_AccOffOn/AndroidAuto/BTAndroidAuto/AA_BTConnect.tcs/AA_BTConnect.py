...
#1.让手机设备处于可发现状态
def Phone_CanBeFind():
    touch(Template('pic/phone_设置.png'), threshold=ST.allSource_threshold, device=DEV3)
    touch(Template('pic/phone_连接.png'), threshold=ST.allSource_threshold, device=DEV3)
    touch(Template('pic/phone_BT.png'), threshold=ST.allSource_threshold, device=DEV3)
...
if exists(Template('pic/BT_Setting.png'), threshold=ST.allSource_threshold, device=DEV1):
    ...
    if exists(Template('pic/蓝牙配对弹框.png'), threshold=ST.allSource_threshold, device=DEV1):
        ...
        #手机侧确认连接
        if exists(Template('pic/phone_蓝牙配对请求.png'), threshold=ST.allSource_threshold, device=DEV3):
        #点击确定
        ...
        #手机侧点击允许
        for i in range(2):
            touch_if(Template('pic/phone_允许.png'), threshold=ST.allSource_threshold,  timeout=10, device=DEV3)
    else:
        error(errType)  
